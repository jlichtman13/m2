...
int main (int argc, char **argv)
{
    ...
    FILE *file;
    file=fopen("ising-demo-metropolis.txt","w");
    int np = 20;
    double tstep = (tmax - tmin) / (np - 1);
    int lx = 64;
    int ly = lx;
    ...
    for (int i = 0; i < np; i++)  //added to run the program multiple times and different vallues
    {
        ...
        report = mcsteps / 32;
	// initialing variables using 0. as a placeholder
...
void print_header (struct ising *model, double tmin, double tmax, FILE * fp)
{
    ...
    fprintf (fp, " Tmin/J = %.3f  Tmax/J = %.3f  H/J = %.3f  %dx%d\n\n", tmin / model->J,
        tmax / model->J, model->H / model->J, model->Lx, model->Ly);
}
